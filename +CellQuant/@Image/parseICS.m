function parseICS(istack)
    ...
    if isunix
        icsfile=[istack.imdir,'/',istack.imfile];
    elseif ispc
        icsfile=[istack.imdir,'\',istack.imfile];
    end
    fid=fopen(icsfile,'r');
    ...
    %istack.dimensions=double(cell2mat([a(4:7)]));
    istack.dimensions=double(cell2mat([a(4:6)]));
    %istack.nch=double(cell2mat(a(6)));
...
% while ~feof(fid)    
%     a=textscan(fid,'%s',1,'Headerlines',0,'delimiter','\n');
%     if ~isempty(a{1});
%         iter=regexp(regexp(a{1}{1},'-it [0-9]+ -mode ','match'),'[0-9]+','match'); %get decon iterations
%         if ~isempty(iter)
